...
int main(int argc, char** argv) {
  ...
  srand(time(NULL));
  vector<complex<double>> random_num_gen{(double) (rand() % 10) + 1};
  ...
  for (auto reverse_block : reversedIndexB) {
    ...
    for (size_t i = 0; i < ids.size(); i++) {
      vector<complex<double>> test = {(double) ids[i]};
